// validate.go - S3M file validator.
// Supports Scream Tracker 3 S3M modules (sample-based playback path).
package s3m
...
type layout struct {
	...
	SignedSamples bool
	Orders        []byte
	ChannelMap    [32]int // physical channel -> mix channel, -1 disabled
	ChannelCount  int
	InsPtrs       []uint16
	PatPtrs       []uint16
}
...
func detectLayout(data []byte) (layout, bool) {
	...
	// S3M file format info byte: 1 => signed sample data, else usually unsigned.
...
func validateInstrument(data []byte, para uint16) string {
	if para == 0 {
		return ""
	}
	off := int(para) << 4
	if off+0x50 > len(data) {
		return "S3M file is truncated in instrument header."
	}
	typ := data[off]
	if typ != 0 && typ != 1 {
		return "S3M contains unsupported instrument type."
	}
	if typ == 0 {
		return ""
	}
	if string(data[off+0x4C:off+0x50]) != "SCRS" {
		return "S3M instrument does not contain SCRS signature."
	}
	...
	if loopBeg > length || loopEnd > length || loopBeg > loopEnd {
		return "S3M instrument has invalid loop bounds."
	}
	...
	if sampleOff+int(length) > len(data) {
		return "S3M file is truncated in sample data."
	}
	return ""
}

func validatePattern(data []byte, para uint16) string {
	if para == 0 {
		return ""
	}
	off := int(para) << 4
	if off+2 > len(data) {
		return "S3M file is truncated in pattern header."
	}
	packed := int(binary.LittleEndian.Uint16(data[off:]))
	if packed < 2 || off+packed > len(data) {
		return "S3M file is truncated in pattern data."
	}
	return ""
}
...
func Validate(data []byte) string {
	...
	if !ok {
		if len(data) < 96 {
			return "Not an S3M file: too short."
		}
	...
	for _, o := range l.Orders {
		...
		if int(o) >= l.PatternCnt {
			return "S3M order list references a non-existent pattern."
		}
	}
	for _, p := range l.InsPtrs {
		if err := validateInstrument(data, p); err != "" {
			return err
		}
	}
	for _, p := range l.PatPtrs {
		if err := validatePattern(data, p); err != "" {
			return err
		}
	}
	return ""
